...
public class TextHandler {
    ...
    public static final Integer MAX_LENGTH_WORD = 15;
    ...
    private static String createSentence(int length) {
        if (length > 1) {
            ...
            if (length > MAX_LENGTH_WORD && length % MAX_LENGTH_WORD == 1) {
                int count = length / (MAX_LENGTH_WORD + 1);
                int ost = length % 16;
                if (ost == 0) {
                    count--;
                }
                for (int i = 0; i < count; i++) {
                    sentence += " " + createWord(MAX_LENGTH_WORD - 1);
                }
                sentence += " " + createWord(7) + " " + createWord(8);
            } else if (length > MAX_LENGTH_WORD && (length % MAX_LENGTH_WORD > 1 || length % MAX_LENGTH_WORD == 0)) {
                int count = length / 16;
                int ost = length % 16;
                if (ost == 0) {
                    count--;
                ...
                for (int i = 0; i < count; i++) {
                    sentence += " " + createWord(MAX_LENGTH_WORD);
                }
            }
    ...
    private static String createSentence(int length, String word) {
        if (length > 1) {
            ...
            if (word.length() + 1 == length) {
                sentence = firstUpperCase(word + TextHandler.createRndMark());
            }
